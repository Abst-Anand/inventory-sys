...
public class ProductDAO {
    ...
    // Method to check for valid product id
    public boolean checkProductId(String prodCode) {
        boolean flag = false;
        try {
            String query = "SELECT * FROM products WHERE pid='" +prodCode+ "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next()) {
                flag = true;
            }
    ...
    public int getProductIdFromCode(String prodCode){
        int flag = -1;
        try {
            String query = "SELECT pid FROM products WHERE productcode='" +prodCode+ "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next()) {
                flag = resultSet.getInt("pid");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean sellProductAndMaintainStock(String productCode, int quantity) {
        ...
        try{
            ...
            if(resultSet.next()) {
                ...
                if(qty < quantity){
                    JOptionPane.showMessageDialog(null, "Insufficient stock for this product: " + productCode);
                    return false;
                }
                String query2 = "UPDATE currentstock SET quantity=quantity-" + quantity +" where productcode='" +productCode+ "'";
    ...
    public ResultSet getQueryResult() {
        try {
            String query = "SELECT pid,productcode,productname,costprice,sellprice,brand FROM products ORDER BY pid";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public ResultSet getPurchaseInfo() {
        try {
            String query = "SELECT fullname,purchaseinfo.ProductCode,ProductName,Quantity,Totalcost, date " +
                    "FROM purchaseinfo INNER JOIN products " +
                    "ON products.productcode=purchaseinfo.productcode "+
                    "INNER JOIN suppliers " +
                    "ON suppliers.suppliercode = purchaseinfo.suppliercode "+
                    " ORDER BY purchaseid;";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    public ResultSet getSalesInfo() {
        try {
            String query = """
                    SELECT fullname,salesinfo.productcode,productname,
                    salesinfo.quantity,revenue,date AS `Sold On`,users.name AS `Sold By`
                    FROM salesinfo INNER JOIN products
                    ON salesinfo.productcode=products.productcode
                    INNER JOIN users
                    ON salesinfo.soldby=users.username
                    INNER JOIN customers
                    ON salesinfo.customercode = customers.customercode;
                    """;
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    public ResultSet getSalesSearch(String text) {
        try {
            String query = "SELECT customers.fullname,salesinfo.productcode,productname,\n" +
                    "                    salesinfo.quantity,revenue,date as `Sold On`,users.name AS `Sold By`\n" +
                    "                    FROM salesinfo INNER JOIN products\n" +
                    "                    ON salesinfo.productcode=products.productcode\n" +
                    "                    INNER JOIN users\n" +
    ...
    public ResultSet getPurchaseSearch(String text) {
        try {
            String query = "SELECT suppliers.fullname,purchaseinfo.productcode,products.productname,quantity,totalcost, date " +
                    ...
                    "WHERE purchaseinfo.productcode LIKE '%"+text+"%' OR productname LIKE '%"+text+"%' " +
    ...
    public DefaultComboBoxModel<String> setComboItems(ResultSet resultSet) throws SQLException {
        Vector<String> prodNames = new Vector<>();
        while (resultSet.next()){
            prodNames.add(resultSet.getString("pid")+ " : " + resultSet.getString("productname"));
        }
        return new DefaultComboBoxModel<>(prodNames);
    }
